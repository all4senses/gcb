 ...
 */
function gcb_html_head_alter(&$head_elements) {
 ...
 */
function gcb_breadcrumb($variables) {
 ...
 */
function gcb_menu_local_tasks(&$variables) {
  $output = '';
 ...
 */
function gcb_process_page(&$variables) {
 ...
 */
function gcb_preprocess_node(&$variables) {
 ...
 */
function gcb_preprocess_region(&$variables, $hook) {
 ...
 */
function gcb_preprocess_block(&$variables, $hook) {
 ...
 *   The name of the template being rendered ("block" in this case.)
 */
function gcb_process_block(&$variables, $hook) {
 ...
 */
function gcb_preprocess_search_block_form(&$vars) {
 ...
 */
function gcb_form_alter(&$form, &$form_state, $form_id) {
...
*/
function gcb_preprocess_views_view_rss(&$vars) {
...
* Default theme function for all RSS rows.
*/
function gcb_preprocess_views_view_row_rss(&$vars) {
  ...
  if (isset($node->body['und'][0]['value'])) {
    $rss_teaser = gcb_misc_getArticleTeaserData('all', $node->body['und'][0]['value'], $vars['row']->nid, 400, TRUE);
    ...
    $rss_teaser = preg_replace('|href="/|', 'href="http://gocloudbackup.com/', $rss_teaser);
  ...
  $vars['item_elements'] = preg_replace('|<dc:creator>.*</dc:creator>|', '<dc:creator>' . gcb_misc_getUserRealName($node->uid) . '</dc:creator>', $vars['item_elements']);
}
 ...
 */
/*
function gcb_preprocess_views_view_unformatted(&$vars) {
 ...
 */
/*
function gcb_captcha($variables) {
 ...
 */
/*
function gcb_preprocess_views_view(&$vars) {
  
  // I set title for preface (at gcb_misc_views_pre_render(&$view)) instead of a view itself.
...
//  }
  ...
  if (isset($vars['view']->name) && ($vars['view']->name == 'blog' || $vars['view']->name == 'news') ) {
    $function = 'gcb_preprocess_views_view_unformatted'; 
...
/*
function gcb_preprocess_html(&$variables) {
